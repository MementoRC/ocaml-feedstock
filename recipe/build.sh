...
_setup_crossopt_env() {
  ...
  # 2026-05-20I: previously intentionally NOT set ("use native linker"). That fails on
  # conda-forge because bare `cc` is not on PATH — only triplet-prefixed gcc is. ocamlc.opt
  # and ocamlopt.opt are BUILD-platform binaries, so CONDA_OCAML_MKEXE must point to the
  # build-platform linker. Prefer NATIVE_MKEXE (set by setup_toolchain), fall back to CROSS_MKEXE,
  # then derive from CONDA_TOOLCHAIN_BUILD as a last resort.
  if [[ -n "${NATIVE_MKEXE:-}" ]]; then
      export CONDA_OCAML_MKEXE="${NATIVE_MKEXE}"
      echo "  [crossopt-env] CONDA_OCAML_MKEXE from NATIVE_MKEXE: ${CONDA_OCAML_MKEXE}"
  elif [[ -n "${CROSS_MKEXE:-}" ]]; then
      export CONDA_OCAML_MKEXE="${CROSS_MKEXE}"
      echo "  [crossopt-env] CONDA_OCAML_MKEXE from CROSS_MKEXE (NATIVE_MKEXE unset): ${CONDA_OCAML_MKEXE}"
  elif [[ -n "${CONDA_TOOLCHAIN_BUILD:-}" ]]; then
      export CONDA_OCAML_MKEXE="${CONDA_TOOLCHAIN_BUILD}-gcc -Wl,-E -ldl"
      echo "  [crossopt-env] CONDA_OCAML_MKEXE derived from CONDA_TOOLCHAIN_BUILD: ${CONDA_OCAML_MKEXE}"
  else
      echo "  [crossopt-env] WARNING: CONDA_OCAML_MKEXE not set; bare 'cc' fallback will likely fail"
  fi
  ...
  echo "  Cross-compiler: ${CROSS_COMPILER_DIR}"
    ...
    if [[ -z "${CONDA_TOOLCHAIN_BUILD:-}" ]]; then
        ...
        case "${build_platform:-${target_platform}}" in
            ...
            linux-s390x)     CONDA_TOOLCHAIN_BUILD="s390x-conda-linux-gnu" ;;
