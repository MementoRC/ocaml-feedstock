...
/* v05_03z: _fpreset no-op. zig's libmingw32 (crt_handler.obj, tlsthrd.obj) references
 * _fpreset via IMAGE_REL_ARM64_BRANCH26 relocation which lld-link can't auto-import
 * for ARM64. Local stub satisfies the relocation as a regular intra-binary branch. */
__attribute__((used, visibility("default")))
void _fpreset(void) {
  /* No-op: floating point reset is safe to skip on ARM64 (no x87 state). */
}
CROSS_WINMAIN_STUB_C
      if "${_zig_exe}" cc -target aarch64-windows-gnu \
        ...
        if [[ "${CROSS_MKEXE}" != *"${_cross_winmain_stub_o}"* ]]; then
          ...
          # v05_03z: Add -Wl,-u,_fpreset to force-keep the _fpreset stub (BRANCH26 reloc from libmingw32).
          CROSS_MKEXE="${CROSS_MKEXE} ${_cross_winmain_stub_o} -Wl,-u,_v05_03y_keepalive -Wl,-u,_fpreset"
          echo "v05_03z: injected WinMain stub + _fpreset no-op + force-keep args into CROSS_MKEXE"
        fi
      else
